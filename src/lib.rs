...
/// Traits for encoding Ciphertexts
pub trait Encode: Obfuscated + Canonical {}

/// Trait implementing support for Obfuscated Encoding
pub trait Obfuscated {
...
/// Trait implementing support for Canonical Encoding
pub trait Canonical {
    /// Expected size of the output object
    type EncodedSize: hybrid_array::ArraySize;
    /// Error returned should decode fail
    type Error: core::error::Error;
    /// Encode this object into its Kemeleon form
    fn as_bytes(&self) -> Array<u8, Self::EncodedSize>;
    /// Decode this object from its encoded form, return `Self::Error` if the
...
impl<U: ArraySize> Canonical for Array<u8, U> {
